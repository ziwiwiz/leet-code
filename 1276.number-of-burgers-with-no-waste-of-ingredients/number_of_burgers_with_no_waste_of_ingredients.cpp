...
public:
    vector<int> numOfBurgers(int tomatoSlices, int cheeseSlices) {
        vector<int> res;
        if(tomatoSlices > 4*cheeseSlices || tomatoSlices < 2*cheeseSlices || 1 == tomatoSlices%2)
            return res;
        int iJumbo = (tomatoSlices - 2*cheeseSlices)/2;
        res.push_back(iJumbo);
        res.push_back(cheeseSlices - iJumbo);
        return res;
    }
};
// @lc code=end
